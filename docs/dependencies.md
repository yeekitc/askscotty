...
| `httpx` | HTTP client for the crawler and the live-tool clients. Gives us timeouts, connection reuse and a shared retry policy without hand-rolling them; also the client both SDKs above already use, so it is in the image regardless. Imported in exactly one place, [`apps/core/http.py`](./backend/apps/core/http.py) — callers go through `get_json`. | `requests` — no timeout by default, which is exactly the failure mode that hangs a demo. `tenacity` for the retry loop — ten lines of `for attempt in range(...)` does not earn a package. |
