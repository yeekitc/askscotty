...
| `react-native-svg` | SVG drawing for the TaskMap component — bezier edges between task nodes, filled/hollow endpoint dots. Expo-managed install, no Xcode/native build step needed beyond what EAS already does. (Note: previously avoided as a *side effect* of react-native-marked; adding it *directly* for a first-party component is a different trade-off.) |
