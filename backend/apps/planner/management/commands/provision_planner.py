...
# The prebuilt toolset only. Our own tools are deliberately *not* declared here:
# every session gets them through `agent_reference`, built from the registry at
# request time, so adding or changing one needs no re-provision and the lanes can
# be built in any order. Declaring them would buy nothing — nothing outside the
# request path can execute a custom tool anyway — at the price of a second place
# for tool definitions to go stale.
